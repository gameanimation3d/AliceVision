...
namespace sfm
{
    ...
    const std::string sTableBegin = "<table border=\"1\">", sTableEnd = "</table>", sRowBegin = "<tr>",
                      sRowEnd = "</tr>", sColBegin = "<td>", sColEnd = "</td>", sNewLine = "<br>", sFullLine = "<hr>";
    ...
    os << "#views: " << sfmData.getViews().size() << sNewLine << " #valid views: " << sfmData.getValidViews().size()
       << sNewLine << " #poses: " << sfmData.getPoses().size() << sNewLine
    ...
    os.str("");
    os << sRowBegin << sColBegin + "IdView" + sColEnd << sColBegin + "Basename" + sColEnd
       << sColBegin + "#Observations" + sColEnd << sColBegin + "Residuals min" + sColEnd
       << sColBegin + "Residuals median" + sColEnd << sColBegin + "Residuals mean" + sColEnd
    ...
    for(sfmData::Views::const_iterator iterV = sfmData.getViews().begin(); iterV != sfmData.getViews().end(); ++iterV)
    {
        ...
        os.str("");
        os << sRowBegin << sColBegin << id_view << sColEnd
            ...
            if(residuals_per_view.find(id_view) != residuals_per_view.end())
            {
                ...
                if(!residuals.empty())
                {
                    ...
                    os << sColBegin << residuals.size() / 2 << sColEnd // #observations
                       << sColBegin << stats.min << sColEnd << sColBegin << stats.median << sColEnd << sColBegin
                       << stats.mean << sColEnd << sColBegin << stats.max << sColEnd;
                }
            }
        }
        os << sRowEnd;
    ...
    {
        IndexT residualCount = 0;
        for(HashMap<IndexT, std::vector<double>>::const_iterator it = residuals_per_view.begin();
            it != residuals_per_view.end(); ++it)
        {
